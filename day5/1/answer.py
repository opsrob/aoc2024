...
then page number 47 must be printed at some point before page number 53. (47 doesn't
necessarily need to be immediately before 53; other pages are allowed to be between them.)

The second section specifies the page numbers of each update. Because most safety
manuals are different, the pages needed in the updates are different too. The first update,
...
In the above example, the first update (75,47,61,53,29) is in the right order:
    ...
    47 is correctly second because 75 must be before it (75|47) and every other page must be
    after it according to 47|61, 47|53, and 47|29.
    61 is correctly in the middle because 75 and 47 are before it (75|61 and 47|61) and 53 and
    29 are after it (61|53 and 61|29).
...
The fourth update, 75,97,47,61,53, is not in the correct order: it would print 75 before 97, which
...
middle page number of each correctly-ordered update. In the above example, the correctly-ordered
updates are:
...
def parse_input(text):
    """seperate rules and updates section of input file"""
...
def solve_puzzle(input_text):
    "solve puzzel and return middle-page sum"
    rules, updates = parse_input(input_text)
    valid_updates = [update for update in updates if is_valid_order(update, rules)]
    return sum(get_middle_page(update) for update in valid_updates), valid_updates

def main(filename):
    """get file and display results"""
    with open(filename, 'r', encoding='us-ascii') as f:
        result, valid_updates = solve_puzzle(f.read())
    print(f'Sum of middle pages: {result}') # should return 6260
    print('Valid updates:', *valid_updates, sep='\n')
