...
long as possible and has an even, gradual, uphill slope. For all practical purposes,
...
will always have height 0. Assembling more fragments of pages, you establish that a
trailhead's score is the number of 9-height positions reachable from that trailhead via
a hiking trail. In the above example, the single trailhead in the top left corner has
...
have scores of 5, 6, 5, 3, 1, 3, 5, 3, and 5. Adding these scores together, the sum of
the scores of all trailheads is 36.

The reindeer gleefully carries over a protractor and adds it to the pile. What is the sum
of the scores of all trailheads on your topographic map?
"""
...
def main():
    ...
    grid = read_input()
    result = solve(grid)
    print(f'Sum of trailhead scores: {result}') # should be 688
